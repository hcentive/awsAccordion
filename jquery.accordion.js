...
(function ($) {
  ...
  var AwsAccordion = function (elem, options) {
    ...
    var defaults = {
      ...
      //Events
      slideOn : 'click.awsAccordion', //Event for Tab slide, can use click or mouseover [string]
      ...
      methods = {
        ...
        calcBordWidthHor : function () {
          ...
          if (!isNaN(parseInt($(headLis).parent().children('li').css('border-right-width'), 10))) {
            borRight = parseInt($(headLis).parent().children('li').css('border-right-width'), 10);
          } else {
            borRight = 0;
          }

          if (!isNaN(parseInt($(headLis).parent().children('li').css('border-left-width'), 10))) {
            borLeft = parseInt($(headLis).parent().children('li').css('border-left-width'), 10);
          } else {
            borLeft = 0;
          }
        ...
        calcDivWidthHor : function () {
          if ($.type(settings.cssAttrsHor.ulWidth) ===  "number") {
            return settings.cssAttrsHor.ulWidth - ($(headLis).parent().children('li').length * (settings.cssAttrsHor.liWidth + methods.calcBordWidthHor()));
          } else {
            //if is 'responsive'
            return horUlParentWidth - ($(headLis).parent().children('li').length * (settings.cssAttrsHor.liWidth + methods.calcBordWidthHor()));
      ...
      core = {
        ...
        bindEvents : function () {
          ...
          headLis.off(settings.slideOn).on(settings.slideOn, core.slideContent);
        ...
        setAttrs : function () {
          ...
          if (settings.type === 'horizontal') {
            ...
            for (i = 0; i < $(headLis).parent().children('li').length; i++) {

              $(headLis).parent().children('li').eq(i).css({
                'width': settings.cssAttrsHor.liWidth + 'px',
                'height': settings.cssAttrsHor.liHeight + 'px'
              }).children('div.hr-slider-content').css({
                'margin-left': settings.cssAttrsHor.liWidth + 'px',
                'width': methods.calcDivWidthHor() + 'px'//,
                //'height': settings.cssAttrsHor.liHeight + 'px'
              }).find('.tab-content:eq(0)').css({
          'margin-left': - (settings.cssAttrsHor.liWidth *(1+i)) + 'px',
          paddingLeft: '15px'
          
        }).animate({width : $('.quotesContainer').width()- 15});
          ...
          if (settings.openCloseHelper) {
            ...
            if (settings.openCloseHelper.numeric) {
              ...
              //Set a icon
               for (i = 0; i <= countHeads; i++) {
                if(!$(headLis[i]).find('i').length){
                    $(headLis[i]).find('h2').append('<i class="icon-' + settings.openCloseHelper.openIcon +'"></i>');
                }
            ...
            h1Height = parseInt($(headLis).children('h2').css("height"), 10) / 2;
            h1HeightIe = parseInt($(headLis).children('h2').css("width"), 10) / 2; //For IE height is not working, we have to use width

            //Set css attrs to Icon, Number and h1
            if (settings.type === 'horizontal') {
              if ((headLis).children('h2').length > 0) {
                //Check if it's IE
                if (versionIE === 7 || versionIE === 8 || versionIE === 9) {
                  $(headLis).children('h2').css({'top': '0px', 'width': 'auto', 'left': ((settings.cssAttrsHor.liWidth / 2) - h1HeightIe) + 'px'});
                } else {
                  $(headLis).children('h2').css('left', ((settings.cssAttrsHor.liWidth / 2) - h1Height) + 'px');
                }
              }
              if (settings.openCloseHelper) {
                if (settings.openCloseHelper.numeric) {
              ...
              if ((headLis).children('h2').length > 0) {
                $(headLis).children('h2').css('padding-top', ((settings.cssAttrsVer.liHeight / 2) - h1Height) + 'px');
              }
              if (settings.openCloseHelper) {
                if (settings.openCloseHelper.numeric) {
        ...
        slideContent : function (ev) {
          ...
          var selector = $(ev.currentTarget),
            visibleElements = $(selector).parent().find('div.hr-slider-content:visible');
          ...
          if (settings.type === 'vertical') {
            if ($(selector).find('div.hr-slider-content').is(":visible")) {
              core.animateSlideUp(selector);
            } else {
              core.animateSlideDown(selector);
            }
          } else {
            if ($(selector).find('div.hr-slider-content').is(":hidden")) {
              settings.startSlide = headLis.index(selector) + 1;
              //Animate Right LI
        ...
        animateSlideRight : function (selector) {
          ...
          $(selector).find('div.hr-slider-content').css('display', 'block');
        },
        ...
        animateSlideLeft : function (selector) {
          ...
          $(selector).parent().find('div.hr-slider-content').animate({width: '0px'}, 'fast', function () {
        ...
        resetSlideContent : function () {
          headLis.find('div.hr-slider-content').hide();
        },
